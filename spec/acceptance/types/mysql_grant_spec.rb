...
describe 'mysql_grant' do
  ...
  describe 'missing privileges for user' do
    ...
    it 'does not find the user' do
      results = run_shell('mysql -NBe "SHOW GRANTS FOR test1@tester"')
  ...
  describe 'missing table for user' do
    ...
    it 'does not find the user' do
      results = run_shell('mysql -NBe "SHOW GRANTS FOR atest@tester"')
  ...
  describe 'adding privileges' do
    ...
    it 'finds the user #stdout' do
      ...
      expect(results.first['result']['stderr']).to contain(%r{GRANT SELECT, UPDATE.*TO 'test2'@'tester'})
    end
  ...
  describe 'adding option' do
    ...
    it 'finds the user #stderr' do
      results = run_shell('mysql -NBe "SHOW GRANTS FOR test3@tester"')
      expect(results.first['result']['stdout']).to be_empty
    end
  end
  ...
  describe 'proxy privilieges' do
    ...
    describe 'adding proxy privileges with other privileges', if: version_is_greater_than('5.5.0') do
      ...
      it 'does not find the user' do
        expect(run_shell('mysql -NBe "SHOW GRANTS FOR proxy2@tester"', acceptable_exit_codes: 1).stderr).to match(%r{There is no such grant defined for user 'proxy2' on host 'tester'})
      end
    end
    ...
    describe 'adding proxy privileges with invalid proxy user', if: version_is_greater_than('5.5.0') do
      ...
      it 'does not find the user' do
        results = run_shell('mysql -NBe "SHOW GRANTS FOR proxy3@tester"')
  ...
  describe 'grants with skip-name-resolve specified' do
    ...
    it 'fails with fqdn' do
      ...
      unless version_is_greater_than('5.7.0')
        results = run_shell('mysql -NBe "SHOW GRANTS FOR test@fqdn.com"')
